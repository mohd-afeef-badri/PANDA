...
def test_mixed_mode():
    ...
    Demonstrates BC priority system: MED groups (highest) > analytical functions
    
    Boundary conditions:
    - left group (MED): Dirichlet u=4*y*(1-y) [parabolic inlet profile]
    - right group (MED): Neumann ∂u/∂n=0.0 [zero flux outlet]
    - bottom boundary (analytical): Dirichlet u=0 [insulated]
    - top boundary (analytical): Dirichlet u=0 [insulated]
    
    Source term: f=0.0 (simple diffusion problem)
    
    Priority verification: MED group definitions take precedence over analytical
    functions, so bottom/top analytical BCs would not override top group BC
    if it existed in MED file. Here, bottom/top are only in analytical since
    square_poly.med doesn't define them as separate groups.
    """
    
    # Load mesh with groups
    mesh = load_med_mesh_mc("./mesh/square_poly.med")
    edge_groups = extract_edge_groups_from_med("./mesh/square_poly.med")
    ...
    # Define BCs using MED groups (highest priority)
    bc_manager.add_bc_by_group("left", "dirichlet", lambda x, y: 4*y*(1-y))
    bc_manager.add_bc_by_group("right", "neumann", 0.0)
    
    # Define BCs using analytical functions (lower priority, only for unmatched edges)
    bc_manager.add_bc_by_function(
        region_func=lambda x, y: abs(y) < 1e-10,
        bc_type="dirichlet",
        value_func=0.0,
    ...
    # Regression test: verify solution values at specific locations
    # Expected values: left wall has parabolic profile (0 at corners, max 1 at center)
    #                  right/top/bottom walls approach 0 (Neumann outlet, Dirichlet insulated)
    expected_values = {
        0: 4.920209e-02,    # Cell 0 centroid (near left, low y)
        1: 3.698635e-03,    # Cell 1 centroid (near right, low y)
        2: 3.697201e-03,    # Cell 2 centroid (near right, high y)
        3: 4.896080e-02,    # Cell 3 centroid (near left, high y)
        4: 2.666662e-02,    # Cell 4 centroid (interior)
    }
    ...
    # Check global statistics - mixed BCs lead to asymmetric solution
    assert np.min(u_dofs) < 0.0, "Solution should have some negative values"
    assert np.max(u_dofs) > 0.0, "Solution should have some positive values"
    assert np.mean(u_dofs) < 0.5, "Solution mean should be moderate due to mixed BCs"
